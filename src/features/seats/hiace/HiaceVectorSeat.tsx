...
interface HiaceVectorSeatProps {
  seat?: Seat | null;
  seatNumber?: string;
  isDriver?: boolean;
  isJumpSeat?: boolean;
...
export function HiaceVectorSeat({
  seat,
  seatNumber,
  isDriver = false,
  isJumpSeat = false,
  ...
  isSelected = false,
  onSelect,
}: HiaceVectorSeatProps) {
  if (isDriver) {
    return (
      <div
        className="relative flex flex-col items-center justify-center w-10 h-11 sm:w-11 sm:h-12 rounded-lg bg-white dark:bg-slate-900 border-2 border-slate-900 dark:border-slate-100 text-slate-900 dark:text-slate-100 select-none"
      ...
      >
        ...
        <span className="text-[7.5px] font-black uppercase tracking-wider text-slate-900 dark:text-slate-100 mt-0.5 leading-none">
          Driver
        </span>
      </div>
    );
  ...
  const sizeClass = isJumpSeat
    ? "w-8 h-10 sm:w-9 sm:h-11"
    : "w-10 h-11 sm:w-11 sm:h-12";
  ...
  return (
    <button
      ...
      className={`group relative flex items-center justify-center transition-all duration-150 select-none ${sizeClass} ${wrapperClass}`}
      aria-label={`Seat ${effectiveNumber} ${seat ? `(${seat.status})` : ""}`}
      title={isJumpSeat ? `Jump Seat ${effectiveNumber}` : `Seat ${effectiveNumber}`}
    >
      <svg
        viewBox="0 0 100 100"
      ...
      <span
        className={`absolute inset-0 flex items-center justify-center text-[10px] sm:text-[11px] font-black leading-none pointer-events-none ${textClass}`}
      >
        {effectiveNumber}
      </span>
    </button>
